...
#define kColumnIdentifierNumber                    @"No"
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    // Setup settings
    self.removeAlert =  [[[NSAlert alloc] init] autorelease];
    [_removeAlert addButtonWithTitle:@"OK"];
    [_removeAlert addButtonWithTitle:@"Cancel"];
    [_removeAlert setShowsSuppressionButton:YES];
//    NSLog(@"Remove alert chck state %d", [[NSUserDefaults standardUserDefaults] boolForKey:kSettingsRemoveCheckKey]);
    if ([[NSUserDefaults standardUserDefaults] boolForKey:kSettingsRemoveCheckKey] == YES) {
        [[_removeAlert suppressionButton] setState:NSOffState];
        [_btnRemoveItemCheckbox setState:NSOnState];
    }
    ...
    for (NSTableColumn *tableColumn in self.resultsTableView.tableColumns ) {
        ...
        SEL compare = nil;
        if ([tableColumn.identifier isEqualToString:kColumnIdentifierCheck] || [tableColumn.identifier isEqualToString:kColumnIdentifierSize] || [tableColumn.identifier isEqualToString:kColumnIdentifierNumber])
    ...
    // Setting drawer
//    NSSize settingVSize = self.settingsView.frame.size;
//    NSLog(@"%@", NSStringFromRect(self.settingsView.frame));
    NSSize drawerSize = NSMakeSize(370, 450);
    [self.settingsDrawer setContentSize:drawerSize];
    [self.settingsDrawer setMinContentSize:drawerSize];
    [self.settingsDrawer setMaxContentSize:drawerSize];
...
- (void)dealloc {
    ...
    self.removeAlert = nil;
    [super dealloc];
}

#pragma mark - Show / Open actions

- (IBAction)settingDrawerToggleAction:(NSButton *)sender {
    NSDrawerState state = [self.settingsDrawer state];
    if (NSDrawerOpeningState == state || NSDrawerOpenState == state) {
        [self.settingsDrawer close];
    } else {
...
- (IBAction)removeItemAction:(NSButton *)sender {
    ...
    if (_arrResults.count > [_resultsTableView selectedRow]) {
        ...
        if ([defaults boolForKey:kSettingsRemoveCheckKey] == YES) {
            [[self.removeAlert suppressionButton] setState:NSOffState];
        }

        self.dicToBeRemoved = [_arrResults objectAtIndex:[_resultsTableView selectedRow]];
        if (self.removeAlert.suppressionButton.state == NSOffState) {
            ...
            [self.removeAlert setMessageText:removeMessage];
            [self.removeAlert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode) {

                if ([[self.removeAlert suppressionButton] state] == NSOnState) {
                    _btnRemoveItemCheckbox.state = NSOffState;
                    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                    [defaults setBool:NO forKey:kSettingsRemoveCheckKey];
...
- (void)runFileSearch {
    ...
    dispatch_group_notify(group, queue, ^ {
        dispatch_async(dispatch_get_main_queue(), ^ {
            ...
            NSString *status = [NSString stringWithFormat:@"Search Completed,\nTotal Found: %ld", (unsigned long)[_arrResults count]];
...
- (void)addNewResult:(NSString *)mFilePath {
    ...
    [self.arrResults addObject:@{kColumnIdentifierNumber: [NSNumber numberWithInteger:_arrResults.count+1],
                                 kColumnIdentifierCheck: [NSNumber numberWithBool:YES],
